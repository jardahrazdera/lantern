// src/systemd.rs
use crate::network::{WifiCredentials, WifiSecurity, Ipv6Config, WireGuardConfig};
use anyhow::Result;
use std::fs;
use std::path::Path;
...
impl SystemdNetworkConfig {
    ...
    pub async fn remove_config(&self, interface: &str) -> Result<()> {
        let config_file = Path::new("/etc/systemd/network").join(format!("10-{}.network", interface));
    ...
    pub async fn create_wifi_config(
        ...
        self.create_wpa_supplicant_config(interface, credentials).await?;
    ...
    async fn create_wpa_supplicant_config(
        ...
        match &credentials.security {
            ...
            WifiSecurity::Enterprise => {
                ...
                return Err(anyhow::anyhow!("Enterprise WiFi requires separate configuration method"));
            }
        }
    ...
    pub async fn disconnect_wifi(&self, interface: &str) -> Result<()> {
        ...
        let wpa_config_file = Path::new("/etc/wpa_supplicant")
    ...
    pub async fn create_ipv6_config(
        ...
        if ipv6_config.enable_ipv6 {
            ...
            config.push_str(&format!("IPv6PrivacyExtensions={}\n", 
                if ipv6_config.privacy_extensions { "yes" } else { "no" }));
        } else {
    ...
    pub async fn configure_ipv6_sysctl(&self, interface: &str, ipv6_config: &Ipv6Config) -> Result<()> {
        ...
        if ipv6_config.enable_ipv6 {
            ...
            Command::new("/usr/bin/sysctl")
                .args(&["-w", &format!("net.ipv6.conf.{}.accept_ra={}", 
                    interface, if ipv6_config.accept_ra { "1" } else { "0" })])
                .output()?;
                
            // Configure privacy extensions
            Command::new("/usr/bin/sysctl")
                .args(&["-w", &format!("net.ipv6.conf.{}.use_tempaddr={}", 
                    interface, if ipv6_config.privacy_extensions { "2" } else { "0" })])
                .output()?;
        } else {
    ...
    pub async fn create_wireguard_from_config_file(&self, config_path: &str, interface_name: &str) -> Result<()> {
    ...
    fn parse_wireguard_config(&self, content: &str, interface_name: &str) -> Result<WireGuardConfig> {
                ...
                match current_section {
                    "Interface" => {
                        match key {
                            ...
                            "DNS" => config.dns.extend(value.split(',').map(|s| s.trim().to_string())),
                    ...
                    "Peer" => {
                        if let Some(ref mut peer) = current_peer {
                            match key {
                                ...
                                "AllowedIPs" => peer.allowed_ips.extend(
