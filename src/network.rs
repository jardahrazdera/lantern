...
impl NetworkManager {
    ...
    fn parse_wifi_scan_results(&self, scan_output: &str) -> Result<Vec<WifiNetwork>> {
        ...
        for line in scan_output.lines() {
            ...
            } else if line.contains("IEEE 802.1X")
                || line.contains("EAP")
                ...
                for addr in addr_info {
                    if addr["family"] == "inet6" {
                        ...
                        addresses.push(Ipv6Address {
                            ...
                            scope,
                            flags,
                            preferred_lifetime: addr["preferred_life_time"]
                                .as_u64()
                                .map(|x| x as u32),
    ...
    async fn get_ipv6_settings(&self, interface: &str) -> Result<(bool, bool, bool)> {
        ...
        if let Ok(content) =
            fs::read_to_string(format!("/proc/sys/net/ipv6/conf/{}/accept_ra", interface))
        {
    ...
    pub async fn configure_ipv6(&self, interface: &str, config: &Ipv6Config) -> Result<()> {
        ...
        systemd_config
            .configure_ipv6_sysctl(interface, config)
            .await?;

        // Create persistent systemd-networkd configuration
        systemd_config
            .create_ipv6_config(interface, config, false, None, None, None)
            .await?;
    ...
    pub async fn add_ipv6_route(
        ...
        let systemd_config = crate::systemd::SystemdNetworkConfig::new();
        systemd_config
            .add_ipv6_route(interface, destination, gateway)
            .await
    }
    ...
    pub async fn destroy_wireguard_interface(&self, interface_name: &str) -> Result<()> {
        ...
        let systemd_config = crate::systemd::SystemdNetworkConfig::new();
        systemd_config
            .remove_wireguard_config(interface_name)
            .await?;
    ...
    fn parse_wireguard_dump(
        ...
        for line in &lines[1..] {
            ...
            if peer_parts.len() >= 4 {
                ...
                let latest_handshake = if peer_parts.len() > 5 && !peer_parts[5].is_empty() {
                    peer_parts[5].parse::<u64>().ok().and_then(|ts| {
                        std::time::UNIX_EPOCH.checked_add(std::time::Duration::from_secs(ts))
                    })
                } else {
                    None
                };
        ...
        let last_handshake = peers.iter().filter_map(|p| p.latest_handshake).max();
    ...
    pub async fn create_hotspot(&self, config: &HotspotConfig) -> Result<()> {
        ...
        let internet_interface =
            self.get_internet_interface()
                .await?
    ...
    pub async fn get_detailed_wifi_info(
        &self,
        interface: &str,
    ) -> Result<Option<DetailedWifiInfo>> {
                                ...
                                if let Some(numerator_str) = quality_str.split('/').next() {
                                    if let Some(denominator_str) = quality_str.split('/').nth(1) {
                                        if let (Ok(num), Ok(den)) = (
                                            numerator_str.parse::<u32>(),
                                            denominator_str.parse::<u32>(),
                                        ) {
                                            if den > 0 {
