...
impl IwdManager {
    ...
    pub async fn get_connection_signal(&self, device_name: &str) -> Result<Option<i16>> {
        ...
        for line in stdout.lines() {
            if line.contains("RSSI") && line.contains("dBm") {
                ...
                if let Some(dbm_part) = line.split_whitespace()
                    .find(|part| part.ends_with("dBm")) {
    ...
    pub async fn connect_to_network(&self, device_name: &str, network_name: &str, passphrase: Option<&str>) -> Result<()> {
    ...
    pub async fn disconnect_device(&self, device_name: &str) -> Result<()> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Failed to disconnect from WiFi: {}", stderr));
        }
    ...
    pub async fn power_device(&self, device_name: &str, powered: bool) -> Result<()> {
        ...
        let output = Command::new("/usr/bin/iwctl")
            .args(&["device", device_name, "set-property", "Powered", power_state])
            .output()
