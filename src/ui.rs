...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let header = Paragraph::new(Line::from(vec![
        Span::styled(format!("{} Lantern", icons::LANTERN), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
...
fn draw_interface_list(f: &mut Frame, app: &App, area: Rect) {
    let interfaces: Vec<ListItem> = app
        ...
        .map(|(i, iface)| {
            ...
            let ip = iface.ipv4_addresses.first()
                .map(|ip| ip.clone())
...
fn draw_interface_details(f: &mut Frame, app: &App, area: Rect) {
    if let Some(interface) = app.get_selected_interface() {
        let mut lines = vec![
            ...
            Line::from(""),
            Line::from(Span::styled("IPv4 Addresses:", Style::default().add_modifier(Modifier::BOLD))),
        ];
        ...
        lines.push(Line::from(vec![
            ...
            Span::raw(interface.gateway.as_ref().map(|s| s.as_str()).unwrap_or("None")),
        ]));

        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled("DNS Servers:", Style::default().add_modifier(Modifier::BOLD))));
...
fn draw_interface_stats(f: &mut Frame, app: &App, area: Rect) {
    if let Some(interface) = app.get_selected_interface() {
        let rx_bytes = Byte::from_u128(interface.stats.rx_bytes as u128).unwrap_or(Byte::from_u64(0));
        let tx_bytes = Byte::from_u128(interface.stats.tx_bytes as u128).unwrap_or(Byte::from_u64(0));
        
        let stats_text = vec![
            Line::from(Span::styled("Network Statistics", Style::default().add_modifier(Modifier::BOLD))),
            Line::from(""),
            Line::from(vec![
                Span::styled(format!("{} RX: ", icons::RX), Style::default().fg(Color::Green)),
                Span::raw(format!("{:.2}", rx_bytes.get_appropriate_unit(byte_unit::UnitType::Binary))),
            ]),
            Line::from(vec![
                Span::raw("  Packets: "),
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled(format!("{} TX: ", icons::TX), Style::default().fg(Color::Blue)),
                Span::raw(format!("{:.2}", tx_bytes.get_appropriate_unit(byte_unit::UnitType::Binary))),
            ]),
            Line::from(vec![
                Span::raw("  Packets: "),
...
fn draw_footer(f: &mut Frame, app: &App, area: Rect) {
    let mut footer_text = vec![
        Span::raw("q: Quit | r: Refresh | e: Edit | u: Up/Down iface | w: WiFi | h: Hotspot | Enter: Details"),
    ];
...
fn draw_edit_dialog(f: &mut Frame, app: &App) {
    ...
    let block = Block::default()
        .title(format!("Edit {}", app.edit_interface.as_ref().unwrap().name))
        .borders(Borders::ALL)
    ...
    let dhcp_text = if app.use_dhcp {
        format!("DHCP: [{}] Enabled (press Space to toggle)", icons::CONNECTED)
    } else {
    ...
    if !app.use_dhcp {
        ...
        let ip = Paragraph::new(app.ip_input.value())
            .block(Block::default().borders(Borders::ALL).title("IP Address").border_style(ip_style));
        ...
        let gateway = Paragraph::new(app.gateway_input.value())
            .block(Block::default().borders(Borders::ALL).title("Gateway").border_style(gw_style));
        ...
        let dns = Paragraph::new(app.dns_input.value())
            .block(Block::default().borders(Borders::ALL).title("DNS Servers (comma separated)").border_style(dns_style));
        f.render_widget(dns, chunks[3]);
    }

    // Instructions
    let instructions = Paragraph::new("Tab: Next field | Space: Toggle DHCP | s: Save | Esc: Cancel")
        .alignment(Alignment::Center);
...
fn draw_wifi_dialog(f: &mut Frame, app: &App) {
    ...
    if app.wifi_scanning {
        networks.push(ListItem::new(format!("{} Scanning for networks...", icons::SCANNING)));
    } else if app.wifi_networks.is_empty() {
        networks.push(ListItem::new(format!("No networks found. Press 'r' to scan. {}", icons::REFRESH)));
    } else {
        for (i, network) in app.wifi_networks.iter().enumerate() {
            ...
            let auto_connect = if in_history {
                if let Some(interface) = app.get_selected_interface() {
                    app.config.get_wifi_profile(&network.ssid, &interface.name)
            ...
            let prefix = if network.connected {
                ...
                if in_history {
                    if auto_connect {
                        format!("{}{}{} ", icons::SELECTED, icons::HISTORY, icons::AUTO_CONNECT)  // Selected + saved + auto
                    } else {
...
fn draw_wifi_connect_dialog(f: &mut Frame, app: &App) {
    ...
    if let Some(network) = &app.selected_wifi_network {
        ...
        let dhcp_text = if app.wifi_use_dhcp { "DHCP (Auto)" } else { "Static IP" };
        let dhcp_input = Paragraph::new(dhcp_text)
            .block(Block::default().borders(Borders::ALL).title("IP Configuration [Space: Toggle]"))
            .style(dhcp_style);
...
fn draw_wifi_loading_dialog(f: &mut Frame, _app: &App) {
    ...
    let loading_text = vec![
        Line::from(""),
        Line::from(vec![
            Span::styled(format!("{} ", icons::SCANNING), Style::default().fg(Color::Yellow)),
            Span::raw("Loading..."),
        ]),
        Line::from(""),
        Line::from(Span::styled("Scanning for networks", Style::default().fg(Color::Gray))),
    ];
...
fn draw_wifi_enterprise_dialog(f: &mut Frame, app: &App) {
    ...
    if let Some(network) = &app.selected_wifi_network {
        ...
        let auth_method = Paragraph::new(auth_method_text)
            .block(Block::default().borders(Borders::ALL).title("Auth Method [1: Cycle]"))
            .style(auth_style);
        ...
        let phase2_enabled = matches!(app.enterprise_auth_method, 
            crate::network::EnterpriseAuthMethod::PEAP | 
            crate::network::EnterpriseAuthMethod::TTLS
        );
        ...
        let phase2_auth = Paragraph::new(phase2_text)
            ...
            .style(if phase2_enabled { phase2_style } else { Style::default().fg(Color::DarkGray) });
        ...
        let identity_input = Paragraph::new(app.enterprise_identity_input.value())
            .block(Block::default().borders(Borders::ALL).title("Identity (optional)"))
        ...
        let ca_cert_input = Paragraph::new(app.enterprise_ca_cert_input.value())
            .block(Block::default().borders(Borders::ALL).title("CA Certificate Path (optional)"))
            .style(ca_cert_style);
        ...
        let client_cert_enabled = matches!(app.enterprise_auth_method, crate::network::EnterpriseAuthMethod::TLS);
        ...
        let client_cert_input = Paragraph::new(app.enterprise_client_cert_input.value())
            .block(Block::default().borders(Borders::ALL).title(client_cert_title))
            .style(if client_cert_enabled { client_cert_style } else { Style::default().fg(Color::DarkGray) });
        ...
        let private_key_enabled = matches!(app.enterprise_auth_method, crate::network::EnterpriseAuthMethod::TLS);
        ...
        let private_key_input = Paragraph::new(app.enterprise_private_key_input.value())
            .block(Block::default().borders(Borders::ALL).title(private_key_title))
            .style(if private_key_enabled { private_key_style } else { Style::default().fg(Color::DarkGray) });
        ...
        let key_pass_enabled = matches!(app.enterprise_auth_method, crate::network::EnterpriseAuthMethod::TLS);
        ...
        let key_pass_input = Paragraph::new(key_pass_text)
            ...
            .style(if key_pass_enabled { key_pass_style } else { Style::default().fg(Color::DarkGray) });
...
fn draw_hotspot_dialog(f: &mut Frame, app: &App) {
    ...
    let ssid_input = Paragraph::new(app.hotspot_ssid_input.value())
        .block(Block::default().borders(Borders::ALL).title("Hotspot Name (SSID)"))
        .style(ssid_style);
    ...
    let password_input = Paragraph::new(password_text)
        .block(Block::default().borders(Borders::ALL).title("Password (min 8 chars)"))
        .style(password_style);
    ...
    let channel_input = Paragraph::new(channel_text)
        .block(Block::default().borders(Borders::ALL).title("WiFi Channel [Space: Cycle]"))
        .style(channel_style);
...
fn draw_wifi_diagnostics_dialog(f: &mut Frame, app: &App) {
    ...
    if let Some(diagnostics) = &app.wifi_diagnostics_data {
        ...
        let connection_info = vec![
            Line::from(Span::styled("🔗 Connection Information", Style::default().add_modifier(Modifier::BOLD).fg(Color::Cyan))),
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(vec![
                Span::styled("Encryption: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::raw(diagnostics.encryption.join(", ")),
            ]),
            Line::from(vec![
                Span::styled("Connected Time: ", Style::default().add_modifier(Modifier::BOLD)),
        ...
        let signal_info = vec![
            Line::from(Span::styled("📶 Signal & Performance", Style::default().add_modifier(Modifier::BOLD).fg(Color::Green))),
            Line::from(""),
            Line::from(vec![
                Span::styled("Signal Strength: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::styled(format!("{} dBm", diagnostics.signal_strength), Style::default().fg(signal_color)),
            ]),
            Line::from(vec![
                Span::styled("Signal Quality: ", Style::default().add_modifier(Modifier::BOLD)),
            ...
            Line::from(vec![
                ...
                Span::raw(format!("{} MHz (Channel {})", diagnostics.frequency, diagnostics.channel)),
            ]),
            Line::from(vec![
                Span::styled("Link Speed: ", Style::default().add_modifier(Modifier::BOLD)),
        ...
        let signal_widget = Paragraph::new(signal_info)
            .block(Block::default().borders(Borders::ALL).title("Signal & Performance"));
        f.render_widget(signal_widget, chunks[1]);

        // Network Statistics Section
        let rx_bytes = byte_unit::Byte::from_u128(diagnostics.rx_bytes as u128).unwrap_or(byte_unit::Byte::from_u64(0));
        let tx_bytes = byte_unit::Byte::from_u128(diagnostics.tx_bytes as u128).unwrap_or(byte_unit::Byte::from_u64(0));
        
        let stats_info = vec![
            Line::from(Span::styled("📊 Network Statistics", Style::default().add_modifier(Modifier::BOLD).fg(Color::Yellow))),
            Line::from(""),
            Line::from(vec![
                Span::styled("📥 RX: ", Style::default().fg(Color::Green)),
                Span::raw(format!("{:.2} ({} packets)", 
            ...
            Line::from(vec![
                ...
                Span::raw(format!("{} errors, {} dropped", diagnostics.rx_errors, diagnostics.rx_dropped)),
            ]),
            Line::from(vec![
                Span::styled("❌ TX Errors: ", Style::default().fg(Color::Red)),
                Span::raw(format!("{} errors, {} dropped, {} retries", 
        ...
        let advanced_info = vec![
            Line::from(Span::styled("🔧 Advanced Details", Style::default().add_modifier(Modifier::BOLD).fg(Color::Magenta))),
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(vec![
                Span::styled("Packet Loss Rate: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::raw({
            ...
            Line::from(vec![
                Span::styled("Connection Quality: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::styled({
        ...
        let advanced_widget = Paragraph::new(advanced_info)
            .block(Block::default().borders(Borders::ALL).title("Advanced Details"));
        ...
        let no_data = vec![
            Line::from(""),
            Line::from(Span::styled("❌ No WiFi Connection Found", 
        ...
        let no_data_widget = Paragraph::new(no_data)
            .block(Block::default().borders(Borders::ALL).title("No Data Available"))
            .alignment(Alignment::Center);
