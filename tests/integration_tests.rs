...
fn test_version_output() {
    ...
    if let Ok(output) = output {
        ...
        assert!(stdout.contains("lantern"), "Version should contain 'lantern'");
...
fn test_help_output() {
    ...
    if let Ok(output) = output {
        ...
        assert!(stdout.contains("network interface"), "Help should mention network interface");
    }
    // If binary doesn't exist, that's OK for CI environments
}
...
fn test_cli_mode_without_root() {
    ...
    if let Ok(output) = output {
        ...
        assert!(!output.status.success(), "Should fail without root privileges");
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.contains("root privileges"), "Should mention root privileges");
    }
    // If binary doesn't exist, that's OK for CI environments
}
...
mod unit_tests {
    ...
    fn test_cargo_metadata() {
        ...
        assert!(cargo_toml.contains("name = \"lantern\""), "Should have correct name");
        assert!(cargo_toml.contains("version = \"0.1.0\""), "Should have version 0.1.0");
        assert!(cargo_toml.contains("jardahrazdera"), "Should have correct repository");
        assert!(cargo_toml.contains("network"), "Should mention network in description");
    }
